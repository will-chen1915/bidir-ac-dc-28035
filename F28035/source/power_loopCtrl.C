...
CTRL2P2Z_coeff CTRL2P2Z_COEFF_VOLTAGE_LOOP_DISCHARGE;
...
Uint16 u16MaxDuty = 0;
Uint16 u16Cnt_RampDuty = 0;

Uint16 u16LVoutRef = 0;
Uint16 u16Cnt_RampRef = 0;
...
void Set_PID_Coff ( void )
{
	...
	CTRL2P2Z_COEFF_VOLTAGE_LOOP_DISCHARGE.inter_A1 = 5000;
	CTRL2P2Z_COEFF_VOLTAGE_LOOP_DISCHARGE.inter_A2 = -3000;
	CTRL2P2Z_COEFF_VOLTAGE_LOOP_DISCHARGE.inter_A3 = 5;
}

void set_MaxDuty(Uint16 u16SetValue)
{
	u16MaxDuty = u16SetValue;
}
...
/*every 500*40us, MAX_DUTY = 352,    352*2000us=7000ms*/
/*every 30*40us=1200us, MAX_DUTY = 352,    352*1.2ms=422ms*/
/*every 22*40us=880us, MAX_DUTY = 352,    352*0.88ms=310ms*/
#if 1
static inline Ctrl_RampMaxDuty()
{
	if(bsoftStart == TRUE)
	{
		u16Cnt_RampDuty++;
		if(u16Cnt_RampDuty >= 22)
	    {
...
/*every 1*40us=40us, u16LVoutRefNormal = 19998,    19998/2*0.04ms=400ms*/

static inline Ctrl_RampLVoltRef()
{
	if(bsoftStart == TRUE)
	{
		u16Cnt_RampRef++;
...
short PI_LVolt ( int16 error, Uint32 Max_out,CTRL2P2Z_coeff* CTRL2p2z,Uint32 U32Min_out )
{
...
interrupt void adc_isr ( void )
{
	...
	if(CHARGE_MODE ==PSUMode_temp)
	{
		i16LVoltError = u16LVoutRef - u16LVoltInst;
		//i16LVoltError = LV_VOLT ( 13.0 ) - u16LVoltInst;
		temp_duty_V = PI_LVolt ( i16LVoltError, ( ((Uint32)u16MaxDuty)<<16 ), &CTRL2P2Z_COEFF_VOLTAGE_LOOP, 0 );	//0x4E1FB1E0 = 20000*2^16
		if(u16LVoltInst > LV_VOLT ( 13.8 ) )
		{
			temp_duty_V = 0;
		}
	...
	else if(DISCHARGE_MODE ==PSUMode_temp)
	{
		i16LVoltError = u16LVoutRef - u16LVoltInst;

		//i16LVoltError = u16LVoltInst - u16LVoutRef;
		temp_duty_V = PI_LVolt ( i16LVoltError, ( ((Uint32)u16MaxDuty)<<16 ), &CTRL2P2Z_COEFF_VOLTAGE_LOOP_DISCHARGE, 0 );	//0x4E1FB1E0 = 20000*2^16

		EPwm5Regs.CMPA.half.CMPA = DC_DC_FREQUENCY - temp_duty_V;
		EPwm6Regs.CMPA.half.CMPA = temp_duty_V;
		EPwm1Regs.CMPB = (temp_duty_V>>1);
	}
	else
	{
...
//		EPwm6Regs.CMPA.half.CMPA = temp_duty_V;
	...
	if ( !Flag_Txd )
	{
		...
		Txd_buf[Flag_Txd_cnt]= ( char ) ( u16LVoutRef>>8 );
		Flag_Txd_cnt++;
		Txd_buf[Flag_Txd_cnt]= ( char ) ( u16LVoutRef&0x00ff );
		Flag_Txd_cnt++;
		Txd_buf[Flag_Txd_cnt]= ( char ) ( u16MaxDuty>>8 );
		Flag_Txd_cnt++;
		Txd_buf[Flag_Txd_cnt]= ( char ) ( u16MaxDuty&0x00ff );
		Flag_Txd_cnt++;
...
{
	Uint16 LV_ref = 0;
	LV_ref = u16LVoutRef + 2;//K16_LV_CHARGE_STEP_SIZE;
